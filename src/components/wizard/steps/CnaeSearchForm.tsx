...
interface CnaeSearchFormProps {
  ...
  onSelectCnae?: (entry: CnaeEntry) => void;
}

export const CnaeSearchForm: React.FC<CnaeSearchFormProps> = ({
  onNext,
  onSelectAgent,
  onSelectCnae,
}) => {
  ...
  const handleSelectCnae = (entry: CnaeEntry) => {
    ...
    if (onSelectCnae) onSelectCnae(entry);
  };
        ...
        {searchTerm.length >= 2 && !selectedCnae && (
          ...
          >
            {filteredCnae.length === 0 ? (
              <div
                style={{ padding: "1rem", color: "#666", textAlign: "center" }}
              >
