...
export const FinalReport: React.FC<FinalReportProps> = ({
  ...
  const getMeasureText = (id: string) =>
    RD_MEASURES.find((m) => m.id === id)?.text || id;
  const getArticle = (id: string) =>
    RD_MEASURES.find((m) => m.id === id)?.article || "";
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              ...
              <span>
                {state.hazard.input.substanceName || "No identificado"}
              </span>

              <strong>Frases H:</strong>
              <span>
                {state.hazard.input.hPhrases.length > 0
                  ? state.hazard.input.hPhrases.join(", ")
                  : "Ninguna"}
              </span>

              <strong>Cas:</strong>
              <span>{state.hazard.input.casNumber || "N/A"}</span>

              <strong>Uso/Origen:</strong>
              <span>
                {state.hazard.input.isMixture ? "Mezcla" : "Sustancia Pura"}{" "}
                {state.hazard.input.concentration
                  ? `(${state.hazard.input.concentration}%)`
                  : ""}
              </span>
              ...
              <span>
                {state.hazard.input.density
                  ? `Densidad: ${state.hazard.input.density} g/cm³`
                  : ""}
                {state.hazard.input.density && state.hazard.input.boilingPoint
                  ? " | "
                  : ""}
                {state.hazard.input.boilingPoint
                  ? `P. Eb: ${state.hazard.input.boilingPoint} ºC`
                  : ""}
                {!state.hazard.input.density && !state.hazard.input.boilingPoint
                  ? "No especificadas"
                  : ""}
            ...
            <div style={{ marginTop: "1rem" }}>
              ...
              <div style={{ marginTop: "0.5rem" }}>
                <TrafficLight
                  ...
                  text={
                    hazardResult?.isHazardous
                      ? "APLICA RD 665/1997 (Agente Cancerígeno/Mutágeno/Reprotóxico)"
                      : "No clasificado como CMR Cat 1A/1B"
                  }
        ...
        >
          ...
          >
            <h4 style={{ marginTop: 0, fontSize: "1rem", color: "#555" }}>
              2.1. Tamiz Cualitativo (Sieve)
            </h4>
            <div
              style={{
                display: "grid",
            ...
            <div style={{ marginTop: "0.5rem" }}>
              <TrafficLight
                status={
                  state.exposureSieve.result?.isRelevant ? "info" : "safe"
                }
                text={
                  state.exposureSieve.result?.isRelevant
                    ? "Exposición Potencialmente Relevante"
                    : "Exposición No Significativa"
                }
          ...
          {/* 2.1b Basic Characterization */}
          {state.hygienicEval.input.basicCharacterization && (
            <div
              style={{
                padding: "1rem",
            ...
            >
              <h4 style={{ marginTop: 0, fontSize: "1rem", color: "#0369a1" }}>
                2.1b. Condiciones de Operación (Caracterización Básica)
              </h4>
              <div
                style={{
                  display: "grid",
              ...
              >
                ...
                <span>
                  {state.hygienicEval.input.basicCharacterization.isOpenProcess
                    ? "Abierto"
                    : "Cerrado"}
                </span>
                ...
                <strong>EPI Respiratorio:</strong>
                <span>
                  {state.hygienicEval.input.basicCharacterization
                    .respiratoryPPE || "No indicado"}
                </span>

                <strong>EPI Dérmico:</strong>
                <span>
                  {state.hygienicEval.input.basicCharacterization.dermalPPE ||
                    "No indicado"}
                </span>
              </div>
            </div>
          ...
          {state.hygienicEval.result && (
            ...
            >
              <h4 style={{ marginTop: 0, fontSize: "1rem", color: "#555" }}>
                2.2. Valoración Higiénica Cuantitativa
              </h4>
              <div
                style={{
                  display: "grid",
              ...
              >
                ...
                <span>
                  {state.hygienicEval.input.vla
                    ? `${state.hygienicEval.input.vla} mg/m³`
                    : "N/A"}
                </span>
                ...
                <TrafficLight
                  ...
                  text={
                    state.hygienicEval.result.isSafe
                      ? "Situación CONFORME (Riesgo Controlado)"
                      : "Situación NO CONFORME (Riesgo Higiénico)"
                  }
