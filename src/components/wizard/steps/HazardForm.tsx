...
export const HazardForm: React.FC<HazardFormProps> = ({ onAnalyze, onNext, initialData }) => {
    ...
    const [detectedComponents, setDetectedComponents] = useState<{ name: string, cat?: string, notes?: string, phrases?: HPhrase[] }[]>([]);
    const [detectedRoutes, setDetectedRoutes] = useState<{ inhalation: boolean, dermal: boolean, oral: boolean, parenteral: boolean }>({ inhalation: false, dermal: false, oral: false, parenteral: false });
    ...
    const analyzeText = (text: string) => {
        ...
        knownCMRs.forEach(cmr => {
            const regex = new RegExp(`(${cmr.regex.source}).{0,100}?(\\d{1,3}(?:[.,]\\d{1,2})?)\\s?%`, 'i');
            ...
            if (match) {
                ...
                const looseMatch = text.substring(text.search(cmr.regex)).substring(0, 150).match(/(\d{1,3}(?:[.,]\d{1,2})?)\s?%/);
                if (looseMatch) {
        ...
        if (foundComponents.length > 0) {
            const unique = Array.from(new Map(foundComponents.map(item => [item.name, item])).values());
            substanceSummary = unique.map(c => `${c.name} (${c.percent > 0 ? c.percent + '%' : 'Conc. desc.'})`).join(', ');
        ...
        const componentsForState = foundComponents.length > 0
            ? foundComponents.map(c => {
                ...
                if (data?.notes?.includes('C1A')) cat = 'Cat. 1A (Cancerígeno Confirmado en Humanos)';
                else if (data?.notes?.includes('C1B')) cat = 'Cat. 1B (Cancerígeno Supuesto en Humanos)';
                else if (CMR_PHRASES.some(h => foundPhrases.includes(h))) cat = 'Posible CMR (Por Frases H globales)';
        ...
        if (/aerosol|niebla|spray/i.test(text)) {
            ...
            if (/vapor|evaporaci[óo]n rápida/i.test(text)) detectedForm = 'liquid_high_volatility';
        ...
        const routes = {
            inhalation: /inhalac|respiratoria|vapor|polvo|aerosol|H350i|H330|H331|H332/i.test(text),
            dermal: /piel|cut[áa]ne|d[ée]rmic|contacto.*piel|H310|H311|H312/i.test(text),
            oral: /ingesti[óo]n|oral|tragar|H300|H301|H302/i.test(text),
            parenteral: /parenteral|inyecci[óo]n|pinchazo|sangre|torrente|aguja/i.test(text)
        };
        // Default Inhalation to true if gas/liquid/dust detected as it's the primary route
        if (detectedForm && detectedForm !== 'solid_massive') routes.inhalation = true;

        setDetectedRoutes(routes);

        setFormData(prev => {
            const chemicalData = lookupChemical(substanceSummary || prev.substanceName);
            return {
                ...prev,
                substanceName: substanceSummary || prev.substanceName || "Sustancia no identificada (Manual)",
                hPhrases: foundPhrases.length > 0 ? foundPhrases : prev.hPhrases,
                isMixture: isMixture,
                concentration: finalConc,
                extraDetails: foundComponents.length > 0 ? JSON.stringify(foundComponents) : undefined,
        ...
        >
            ...
            <div
                ...
                style={{
                    ...
                    padding: '1.5rem',
                    border: isDragging ? '2px dashed var(--color-primary-dark)' : '2px dashed var(--color-primary)',
                    borderRadius: '8px',
                    textAlign: 'center',
            ...
            >
                ...
                <label
                    ...
                    style={{
                        ...
                        padding: '0.5rem 1rem',
                        backgroundColor: analysisStatus === 'analyzing' ? '#ccc' : 'var(--color-white)',
                ...
                >
                    {analysisStatus === 'analyzing' ? 'Analizando Ficha...' : 'Subir Ficha de Seguridad (PDF)'}
                </label>
                {analysisStatus === 'success' && (
                    <div style={{ marginTop: '0.5rem', color: 'green', fontWeight: 'bold' }}>✅ Datos extraídos correctamente</div>
                )}
                {analysisStatus === 'error' && (
                    <div style={{ marginTop: '0.5rem', color: 'red', fontWeight: 'bold' }}>❌ Error al leer el PDF</div>
                )}
            </div>

            <div style={{ display: 'flex', alignItems: 'center', margin: '1rem 0', color: '#ccc' }}>
                <div style={{ flex: 1, height: '1px', background: 'currentColor' }}></div>
                <span style={{ padding: '0 1rem', fontSize: '0.8rem' }}>O INTRODUCCIÓN MANUAL</span>
                <div style={{ flex: 1, height: '1px', background: 'currentColor' }}></div>
            </div>

            <div className="form-group mb-2">
                <label style={{ display: 'block', fontWeight: 600, marginBottom: '0.5rem' }}>Agente(s) cancerigeno(s) identificado(s)</label>
                <input
                    type="text"
                ...
                {detectedComponents.length > 0 ? (
                    <div style={{ marginTop: '1rem' }}>
                        {detectedComponents.map((comp, idx) => (
                            <div key={idx} style={{ marginBottom: '0.75rem', padding: '0.75rem', backgroundColor: '#fff8f1', borderLeft: '4px solid #dc2626', borderRadius: '4px' }}>
                                ...
                                {comp.notes && <div style={{ fontSize: '0.75rem', color: '#666', marginTop: '0.25rem' }}>Notas: {comp.notes}</div>}
                            </div>
                        ))}
                    </div>
                ) : (
                    ...
                    formData.hPhrases.length > 0 && (() => {
                        ...
                        const getExplanation = (phrases: HPhrase[]) => {
                            ...
                            if (phrases.some(p => ['H340', 'H350', 'H360FD'].includes(p))) {
                                ...
                                text = 'Sustancia de la que se sabe o se supone que es carcinógena, mutágena o tóxica para la reproducción en humanos. REQUIERE SUSTITUCIÓN PRIORITARIA.';
                                color = '#dc2626'; // Red
                            } else if (phrases.some(p => ['H351', 'H341', 'H361'].includes(p))) {
                                cat = 'Categoría 2 (Sospechoso)';
                                text = 'Se sospecha que provoca cáncer, defectos genéticos o daña la fertilidad/feto. Requiere vigilancia.';
                                color = '#d97706'; // Amber
                            } else {
                        ...
                        return (
                            <div style={{ marginTop: '0.5rem', padding: '0.75rem', backgroundColor: '#fff8f1', borderLeft: `4px solid ${info.color}`, borderRadius: '4px' }}>
                                <div style={{ fontWeight: 'bold', color: info.color }}>{info.cat}</div>
            ...
            <div className="form-group mb-4">
                ...
                <div className="routes-grid">
                    <div style={{
                        ...
                        borderRadius: '6px',
                        textAlign: 'center',
                        backgroundColor: detectedRoutes.inhalation ? '#e0f2fe' : '#f3f4f6',
                        border: detectedRoutes.inhalation ? '2px solid #0ea5e9' : '1px solid #e5e7eb',
                    ...
                    <div style={{
                        ...
                        border: detectedRoutes.dermal ? '2px solid #d97706' : '1px solid #e5e7eb',
                    ...
                    <div style={{
                        ...
                        border: detectedRoutes.oral ? '2px solid #e11d48' : '1px solid #e5e7eb',
                    ...
                    <div style={{
                        ...
                        borderRadius: '6px',
                        textAlign: 'center',
                        backgroundColor: detectedRoutes.parenteral ? '#f3e8ff' : '#f3f4f6',
                        border: detectedRoutes.parenteral ? '2px solid #9333ea' : '1px solid #e5e7eb',
                        opacity: detectedRoutes.parenteral ? 1 : 0.6
                    }}>
                        <div style={{ fontSize: '1.5rem', marginBottom: '0.25rem' }}>🩹</div>
                        <div style={{ fontSize: '0.85rem', fontWeight: 600, color: detectedRoutes.parenteral ? '#7e22ce' : '#6b7280' }}>Parenteral</div>
                    </div>
                </div>
            </div>

            <div className="form-group mb-2">
                <label style={{ display: 'block', fontWeight: 600, marginBottom: '0.5rem' }}>Frases H Identificadas (Sección 2 FDS)</label>
            ...
            <div className="form-group mb-4">
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                    <input
                        ...
                        onChange={e => setFormData({ ...formData, isMixture: e.target.checked })}
                    />
            ...
            <div className="actions" style={{ marginTop: 'var(--spacing-lg)', borderTop: '1px solid #eee', paddingTop: 'var(--spacing-md)' }}>
                {!result ? (
                    ...
                    <div className={`result-box ${result.isHazardous ? 'danger' : 'safe'}`} style={{
                        padding: '1rem',
                        backgroundColor: result.isHazardous || result.requiresZeroExposure ? '#fff3cd' : '#d4edda',
                        ...
                        {result.justifications.map((just, idx) => (
                            <div key={idx} style={{ marginTop: '1rem', borderTop: idx > 0 ? '1px dashed rgba(0,0,0,0.1)' : 'none', paddingTop: idx > 0 ? '0.5rem' : '0' }}>
                                <p style={{ margin: '0.5rem 0', fontSize: '0.9rem' }}>{just.technical}</p>
                                <div style={{
                                    fontSize: '0.8rem',
                                    fontStyle: 'italic',
                                    ...
                                    <strong>Base Legal:</strong> {just.legal.article} - {just.legal.text}
                                </div>
                            </div>
                        ))}
                        ...
                        <button
                            ...
                            style={{
                                marginTop: '1rem',
                                backgroundColor: result.isHazardous || result.requiresZeroExposure ? 'var(--color-danger)' : 'var(--color-safe)',
                                color: 'white',
                                border: 'none',
                                padding: '0.5rem 1rem',
                                borderRadius: '4px',
