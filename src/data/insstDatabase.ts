export interface ChemicalData {
  ...
  loq_ug?: number; // Analytical Limit of Quantitation in µg (Mass)
  sampling: {
...
export const INSST_DATABASE: Record<string, ChemicalData> = {
  Formaldehído: {
    ...
    loq_ug: 0.1, // HPLC-UV sensitivity is high
    sampling: {
  ...
  "Sílice Cristalina": {
    ...
    suggestedLod: 0.005,
    loq_ug: 10.0, // DRX filter analysis typically 10-20ug
    sampling: {
...
export function lookupChemical(nameRaw: string): ChemicalData | null {
  ...
  if (target.includes("silice") || target.includes("sílice"))
    return INSST_DATABASE["Sílice Cristalina"];
  if (target.includes("dicromato"))
    return INSST_DATABASE["Dicromato de Potasio"];
  if (target.includes("etilen") || target.includes("oxido"))
    return INSST_DATABASE["Óxido de Etileno"];
