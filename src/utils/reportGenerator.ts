...
export const generateReportData = (state: AssessmentState): ReportData => {
  ...
  if (lepData?.notes.some((n) => ["C1A", "C1B"].includes(n)) || hazard.hPhrases.some(h => ["H350", "H350i"].includes(h))) {
  ...
  const volatility = hazard.vapourPressure ? `${hazard.vapourPressure} Pa` : "volatilidad no determinada";
  ...
  const narrative = `La tarea evaluada, "${taskName}", implica la manipulación del agente en estado ${statePhys} con una ${
    hazard.vapourPressure ? `presión de vapor de ${volatility}` : "volatilidad estimada"
  }. El proceso se desarrolla en un régimen de ${procType}, manipulándose ${quantity}. La frecuencia de exposición es ${frequency} con una duración por ciclo de ${duration}.`;
  ...
  const isScenarioA =
    basic?.technicalMeasure === "closed_system" || state.hygienicEval.result?.isSafe;
  ...
  if (isScenarioA) {
    ...
    conclusionType = "B";
    const vlaRef = lepData?.vla_ed ? `${lepData.vla_ed} mg/m³` : "el VLA aplicable";
